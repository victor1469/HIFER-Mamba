...
class PromptGenerator(nn.Module):
    """
    Generate dynamic prompts (full_embedding) from context tokens via cross-attention.

    Inputs:
        - context: (B, N, C_ctx)  # tokens from current layer
    Outputs:
        - full_embedding: (B, num_tokens, d_state)

    Config:
        - num_tokens: number of prompts/prototypes (matches ASSM.embeddingB.num_embeddings)
        - d_state: target prompt dimension (match ASSM.d_state)
        - qk_dim: attention key/query dim (internal)
    """
    def __init__(self, num_tokens, d_state, context_dim, qk_dim=None):
        super().__init__()
        self.num_tokens = num_tokens
        self.d_state = d_state
        self.context_dim = context_dim
        self.qk_dim = qk_dim or max(16, min(context_dim // 2, d_state))

        # learnable base prompts (as queries)
        # shape (num_tokens, qk_dim)
        self.base_prompts = nn.Parameter(torch.randn(num_tokens, self.qk_dim) * 0.02)

        # project base prompts -> queries (optional linear)
        self.q_proj = nn.Linear(self.qk_dim, self.qk_dim, bias=False)

        # project context tokens -> keys and values
        self.k_proj = nn.Linear(context_dim, self.qk_dim, bias=False)
        # values projected to d_state to match ASSM prompt dim
        self.v_proj = nn.Linear(context_dim, d_state, bias=False)

        # optional small temperature / scaling parameter
        self.scale = (self.qk_dim ** -0.5)

        # small MLP to optionally refine full_embedding (residual)
    ...
    def forward(self, context):
        """
        context: (B, N, C_ctx)
        returns full_embedding: (B, num_tokens, d_state)
        """
        B, N, C = context.shape
        # keys & values
        K = self.k_proj(context)            # (B, N, qk_dim)
        V = self.v_proj(context)            # (B, N, d_state)

        # queries: from base prompts
        Q = self.q_proj(self.base_prompts)  # (num_tokens, qk_dim)
        Q = Q.unsqueeze(0).expand(B, -1, -1)  # (B, num_tokens, qk_dim)

        # attention
        # (B, num_tokens, qk_dim) @ (B, N, qk_dim).transpose -> (B, num_tokens, N)
        attn = torch.matmul(Q, K.transpose(-2, -1)) * self.scale
        attn = torch.softmax(attn, dim=-1)  # (B, num_tokens, N)

        # aggregate
        full_embedding = torch.matmul(attn, V)  # (B, num_tokens, d_state)

        # optional refine (residual)
        full_embedding = full_embedding + self.refine(full_embedding)

        return full_embedding  # (B, num_tokens, d_state)
...
class ASSM(nn.Module):
    def __init__(self, dim, d_state, input_resolution, num_tokens=64, inner_rank=128, mlp_ratio=2.):
        ...
        #1031-add this sequence
        self.nl = nn.Sequential(
            nn.Linear(hidden, max(hidden // 2, 16)),  # bottleneck 可选，减小参数
            nn.GELU(),
            nn.Linear(max(hidden // 2, 16), hidden),
        )
        ...
        #1029-1
        self.freq_branch = FrequencyBranch(num_channels=hidden)  # hidden = int(self.dim * self.expand)
        ...
        #1031-1
        self.nl_scale = nn.Parameter(1e-2 * torch.ones(1, 1, hidden))

    #1030--The function parameter is replaced with the next line.
    #def forward(self, x, x_size, token):  
    def forward(self, x, x_size, token=None, full_embedding=None):
    
        B, n, C = x.shape
        H, W = x_size

        #1030--Replace this statement with one of the following 11 compatibility conditional statements.
#         full_embedding = self.embeddingB.weight @ token.weight  # [128, C]
        if full_embedding is not None:
            # allowed shapes:
            # (B, num_tokens, d_state) -> use directly
            # (num_tokens, d_state) -> expand to batch
            if full_embedding.dim() == 2:
                # (num_tokens, d_state)
                full_embedding = full_embedding.unsqueeze(0).expand(B, -1, -1)
            elif full_embedding.dim() == 3:
                # (B, num_tokens, d_state) ok
                pass
            else:
                raise ValueError("full_embedding must be shape (num_tokens,d_state) or (B,num_tokens,d_state)")
        else:
            # original behavior: needs `token` (nn.Embedding)
            assert token is not None, "Either token or full_embedding must be provided to ASSM"
            full_embedding = self.embeddingB.weight @ token.weight  # (num_tokens, d_state)
            full_embedding = full_embedding.unsqueeze(0).expand(B, -1, -1)  # expand batch
        ...
        #1029-1
        x = self.freq_branch(x)  # (B, hidden, H, W)
        cc = x.shape[1]
...
class AttentiveLayer(nn.Module):
    ...
    def forward(self, x, x_size, params):
        ...
        shortcut = x
        #1030-1--Replace this line with the following line, adding an additional `full_embedding` parameter.
#         x_aca = self.assm(self.norm3(x), x_size, self.embeddingA) + x
        x_aca = self.assm(self.norm3(x), x_size, token=self.embeddingA, full_embedding=params.get('full_embedding', None)) + x
...
class FrequencyBranch(nn.Module):
    def __init__(self, num_channels, hidden_dim=None):
        super().__init__()
        hd = hidden_dim or max(64, num_channels // 2)
        # 小 mlp: channel descriptor -> per-channel scale
        self.mlp = nn.Sequential(
            nn.Linear(num_channels, hd),
            nn.SiLU(),
            nn.Linear(hd, num_channels),
        )
        # 初始化小幅度 (容易训练)
    ...
    def forward(self, x_spatial):
        """
        x_spatial: (B, C, H, W), dtype=float
        returns: x_spatial_mod: (B, C, H, W)
        """
        # 1) compute rfft magnitude
        # rfft2 returns complex tensor with shape (B, C, H, Wf)
        F = torch.fft.rfft2(x_spatial, dim=(-2, -1))
        mag = torch.abs(F)  # (B, C, H, Wf)
        # 2) compress frequency dims -> descriptor per (B,C)
        desc = torch.mean(torch.log1p(mag), dim=(-2, -1))  # (B, C)
        # 3) predict per-channel gates
        gate = torch.sigmoid(self.mlp(desc))  # (B, C)
        gate = gate.view(gate.shape[0], gate.shape[1], 1, 1)  # (B, C, 1, 1)
        # 4) residual modulation
        x_mod = x_spatial * (1.0 + gate)
        return x_mod
#1029


@ARCH_REGISTRY.register()
class Mambafpuao(nn.Module):
    def __init__(self,
                 img_size=64,
                 patch_size=1,
                 in_chans=3,
        ...
        #1128-1
        self.prompt_share_alpha = nn.Parameter(torch.tensor(0.1))
        ...
        #1110---2 inside sentence
        self.prompt_proj = nn.Sequential(
            nn.Linear(d_state, self.embed_dim),
            nn.SiLU(),
        ...
        #--2 long sentence 1110
            
        for i_layer in range(self.num_layers):
            pg = PromptGenerator(num_tokens=num_tokens, d_state=d_state, context_dim=self.embed_dim)
            self.prompt_gens.append(pg)
        #---1030
    ...
    def forward_features(self, x, params):
        ...
        #1128-1
        prev_full_embedding = None
            
        for i_layer, layer in enumerate(self.layers):
            # generate dynamic full_embedding from current context tokens x
            # x: (B, N, C)
            full_embedding = self.prompt_gens[i_layer](x)  # (B, num_tokens, d_state)
            #1128-3
            if prev_full_embedding is not None:
                full_embedding = full_embedding + self.prompt_share_alpha * prev_full_embedding
            prev_full_embedding = full_embedding.detach()  # update for next
            # copy params to avoid side effects
            params_local = dict(params)
            params_local['full_embedding'] = full_embedding
            # pass params_local into layer; layer (AttentiveLayer) will forward to ASSM which will accept it
            x = layer(x, x_size, params_local)

        x = self.norm(x)  # b seq_len c
        #1110---9
        try:
            fe_mean = full_embedding.mean(dim=1)  # (B, d_state)
            fe_proj = self.prompt_proj(fe_mean)  # (B, embed_dim)
            N = x.size(1)
            fe_add = fe_proj.unsqueeze(1).expand(-1, N, -1).contiguous()  # (B, N, embed_dim)
            alpha = 0.05  
            x = x + alpha * fe_add
